...
if (sessionStorage.getItem(CART)) {
    giohang = JSON.parse((_a = sessionStorage.getItem(CART)) !== null && _a !== void 0 ? _a : "[]");
}
function addCart(x) {
    var _a;
    const pa = (_a = x.parentNode) === null || _a === void 0 ? void 0 : _a.parentNode; // Lấy ra Node cha (kiểm tra null)
    if (pa) {
        const hinh = pa.children[0].children[0]
            .src;
        console.log(hinh);
...
function tangsl(x) {
    ...
    if (totalPriceElement) {
        totalPriceElement.children[0].innerText = `${formatNumber(tinhTongTien())}đ`;
    }
}
...
function xoaSanPham(index) {
    ...
    if (giohang.length === 0) {
        if (totalPriceElement) {
            totalPriceElement.children[0].innerText = "0đ";
        }
    }
    else {
        if (totalPriceElement) {
...
function showCart() {
    ...
    if (gh && gh.length > 0) {
        ...
        soluongspCart();
        document.getElementById("total-price").children[0].innerText = `${formatNumber(tinhTongTien())}đ`;
    }
    else {
