...
$(document).ready(function () {
    ...
    const listStatus: JQuery<HTMLElement> = $("#listStatus");
    let listS: string = "";
    listS += `
            <a
    ...
    try {
        const placeholder: JQuery<HTMLElement> = $("#placeholder_order");
        // lấy danh sách categories qua api
        cartService.findAllCart().then((data: any) => {
            console.log(data);
            let list: string = "";
            var stt: number = 0;
            for (const key in data) {
                ...
                const statusText: string = statusMap[status]; // Ánh xạ giá trị trạng thái từ số sang chuỗi
                // console.log(items);
                list += `
                    <tr>
                        <td>
    ...
    function countOrdersByStatus(status: string): Promise<number> {
        return new Promise((resolve, reject) => {
            cartService
                .findAllCart()
                .then((data: any) => {
                    const count: number = data.reduce(
                        (acc: number, cart: any) => {
